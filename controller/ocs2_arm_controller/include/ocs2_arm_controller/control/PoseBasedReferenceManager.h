...
public:
   ...
   */
  void setCurrentEndEffectorPoses(const vector_t &left_ee_pose,
...
private:
   ...
   *
   * 在双臂模式下，总是同时更新两个臂的轨迹。
   * 在单臂模式下，previous_right_target_state
   * 参数不会被使用（可以传递零向量）。
   *
  ...
  // PoseStamped 经过 TF 转换后的 Pose 处理（与 Pose
  // 回调分开，便于区分旧/新接口）
  ...
  // 发布当前目标
  // @param arm_type 指定发布哪个臂的目标："left" 只发布左臂，"right"
  // 只发布右臂，空字符串或"both" 发布两个臂（仅双臂模式）
  ...
  rclcpp::Subscription<arms_ros2_control_msgs::msg::CircleMessage>::SharedPtr
   ...
   //新增圆弧callback
   //单纯规划，不管坐标系
  void transCircleMessageToBaseFrame(arms_ros2_control_msgs::msg::CircleMessage::SharedPtr msg, arms_ros2_control_msgs::msg::CircleMessage::SharedPtr base_msg);
  bool initCircleCurve(vector_t start_pose, arms_ros2_control_msgs::msg::CircleMessage::SharedPtr msg, std::shared_ptr<planning::CircularCurver> circle_ptr);
