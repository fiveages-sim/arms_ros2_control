...
namespace ocs2::mobile_manipulator
{
    ...
    void PoseBasedReferenceManager::pathCallback(const nav_msgs::msg::Path::SharedPtr msg)
    {
        if (!dual_arm_mode_)
        {
            RCLCPP_WARN(logger_, "Path callback called but dual arm mode is not enabled");
            return;
        }
        ...
        for (size_t i = 0; i < kNumSamples; ++i)
        {
            ...
            // 合并左右臂状态
            vector_t combined_state = vector_t::Zero(14);
        ...
        RCLCPP_INFO(logger_,
                    "处理路径：左臂 %zu 个路径点，右臂 %zu 个路径点，生成 %zu 个轨迹点",
