...
SONY_DUALSHOCK4_CONFIG = 'sony_dualshock4'
GENERIC_PS4_CONTROLLER_CONFIG = 'generic_ps4_controller'
LEGACY_CONFIG_ALIASES = {
    'gamesir_two': SONY_DUALSHOCK4_CONFIG,
    'gamesir_two_arm': GENERIC_PS4_CONTROLLER_CONFIG,
}
...
def _ps4_controller_config_name():
    return GENERIC_PS4_CONTROLLER_CONFIG if _is_arm_architecture() else SONY_DUALSHOCK4_CONFIG


def _ps4_mapping_config_name_from_device_name(name):
...
def _detect_config_name(config_value, config_dir, joy_dev):
    ...
    if requested and requested.lower() != 'auto':
        requested_config = LEGACY_CONFIG_ALIASES.get(requested.lower(), requested)
        if requested_config != requested:
            print(f"[INFO] Joystick config '{requested}' is deprecated; using '{requested_config}' instead.")
        requested_path = os.path.join(config_dir, f'{requested_config}.yaml')
        if os.path.exists(requested_path):
            print(f"[INFO] Using explicit joystick config: {requested_config}")
    ...
    if enumerated_name and enumerated_name not in match_sources:
    ...
    for source in match_sources:
        ...
        ps4_mapping_config_name = _ps4_mapping_config_name_from_device_name(source)
        if ps4_mapping_config_name:
            config_path = os.path.join(config_dir, f'{ps4_mapping_config_name}.yaml')
            if os.path.exists(config_path):
                print(f"[INFO] Auto-selected joystick config '{ps4_mapping_config_name}' for device '{source}'")
                return ps4_mapping_config_name
