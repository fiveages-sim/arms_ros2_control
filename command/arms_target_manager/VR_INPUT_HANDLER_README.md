...
左右 `/teleop/*_ee_pose` 回调只更新输入缓存及接管状态。一个 10 ms wall timer
使用最新输入计算并发布 `/left_target`、`/right_target`；有效 UPDATE 期间目标频率
为 100 Hz，实际间隔受 executor 调度影响。未收到新输入的短间隔内继续输出；
...
旧 `applyStaleCatchUpRamp()` 调用已停用，实现暂留供第二阶段替换。
因此第一阶段只稳定发送节奏，新输入仍可能引起位置/姿态阶跃，不提供平滑或速度限制。
