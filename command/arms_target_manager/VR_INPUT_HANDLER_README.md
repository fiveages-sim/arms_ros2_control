...
- 在 `vr_follow_frame` 中使用 `p = p_robot_base + p_head - p_head_base`、`q = q_head * inverse(q_head_base) * q_robot_base`；最后通过 TF 转到 `head_current_pose.header.frame_id` 发布，与双臂从 current pose 获取发布坐标系的方式一致。
- 每次接管重新记录头显基准。机器人基准优先使用最后发布的目标（保存于明确的控制坐标系，再转换到计算坐标系）；只有没有历史目标时才用当前实际位姿。暂停、退出模式、断流不清除历史目标，避免柔顺控制的实际偏差累积到目标中。
- 每条输入均检查有限值和四元数有效性，不做位姿超时检测。头显消息停止时不更新目标；消息恢复且控制状态未改变时，继续使用原基准，不因消息间隔自动重新对齐。机器人实际位姿只缓存，不按接收时间判定过期。
- VR 实际接管期间，头部 marker 保留最终目标显示，但拖动和菜单发送均被禁用；释放后恢复交互。停止发布不取消 WBC 已收到的目标。
...
左右 `/teleop/*_ee_pose` 回调只更新输入缓存及接管状态。一个 5 ms wall timer
使用最新输入计算并发布 `/left_target`、`/right_target`；有效 UPDATE 期间目标频率
为 200 Hz，实际间隔受 executor 调度影响。未收到新输入的短间隔内继续输出；
若摇杆、坐标变换和模式也未改变，重复的就是相同目标。

每侧输入超过 200 ms 未更新，暂停该侧输出，另一侧独立运行。新输入恢复后，
从机器人当前位姿重建该侧 VR 基准，清除摇杆累计偏移；暂停状态保留。
未收到首次输入、STORAGE、控制禁用、单臂被抑制或 TF 未就绪时不发布对应目标。
停止发布沿用下游控制器行为，不等同于急停。

第一阶段只稳定发送节奏，新输入仍可能引起位置/姿态阶跃。
头部、夹爪、底盘、腰部、校准和状态命令均保留原触发方式。

## 双臂目标统一平滑（第二阶段）

## 双臂目标统一平滑（第二阶段：两级一阶串联）

固定输出路径中，每臂在 `vr_follow_frame` 内对计算目标做**两级一阶平滑**，再转换到
current pose 坐标系发布（`smoothTarget()`）：

- 一级：`s1 += alpha * (raw - s1)`，`q1 = slerp(q1, q_raw, alpha)`
- 二级：`s2 += alpha * (s1 - s2)`，`q2 = slerp(q2, q1, alpha)`，输出取 `s2` / `q2`
- `alpha = 1 - exp(-dt / (tau/2))`（每级时间常数 `tau/2`），`dt` 上限 10 ms（约两个输出周期）

**为什么要两级**：上游 VR 是 **约 98 Hz 且抖动很大**的零阶保持流（实测 bag
`20260914_160212`：dt 中位 10.2 ms、p95 25 ms、最小 1.5 ms）。单级（指数）或
“线性插值斜坡”的**速度**在每个新采样处都会阶跃，表现为“分段恒速 + 每次跳变”，
就是 target 曲线上一眼能看到的锯齿。实测（同一段 bag，200 Hz tick，tau=50 ms）：

| 实现 | tick 间速度变化 med / p90 / max | 位置二阶差分 p99 | 15 Hz 衰减 |
|---|---|---|---|
| 零阶保持（不过滤） | 25% / 77% / 154% | 4594 µm | 1.00 |
| 线性插值斜坡（旧实现） | 0.0% / 16% / 303% | 243 µm | 0.21 |
| 单级一阶 | 10% / 20% / 328% | 222 µm | 0.21 |
| **两级一阶（现实现）** | 4% / **11%** / 122% | **60 µm** | **0.15** |

两级串联后第二级的输入是第一级的连续输出，位置曲线变成 C1（速度连续），所以在同等低频
滞后（合计 = `tau`）下锯齿消失、二阶差分小 4 倍，15 Hz 抑制也更好。

也测过“按采样时刻做真正的插值重采样”：实时只拿得到过去的采样，alpha 恒被截断到 1
（等价于零阶保持，不做任何插值）；真要插值必须多等一个采样周期，而该流实测 p95 达
25 ms，对遥操作太重。所以输入保持零阶保持，台阶交给两级滤波处理。

在计算坐标系中平滑，底盘运动时目标仍随车体移动，不会在 world 中拖尾。
进入 UPDATE、禁用、校准、控制拓扑变化、HOLD→OCS2、输入超时恢复、WBC 单臂重建和
双臂解耦重建都会清空平滑状态，首帧直接输出新基准对应的目标（不追赶）。

参数 `vr_target_smoothing_tau_s`（double，秒，默认 `0.05`）**运行时可调**，不再只读：

```bash
ros2 param set /arms_target_manager vr_target_smoothing_tau_s 0.01
```

`0` 表示旁路（直接输出目标）；负值或非有限值在启动时导致节点启动失败，运行时由
参数回调拒绝并给出失败原因。滤波器形式下改 `tau` 本身就是连续的，不需要重建状态。
`tau` 是两级低频滞后的合计值（每级 `tau/2`），按时间定义、与输出频率无关。

这是目标跟随平滑，会增加延迟，不提供速度、加速度或加加速度限制。

下表是更早的指数（单级一阶）实现测得的，只作历史参考。
离线回放（bag `20260911_113809`，按定时周期零阶保持重采样已录制的 target，
再应用滤波；只反映算法效果，未包含 TF、executor 调度和真实设备）：

5 ms 周期、dt 上限 10 ms（旧实现）：

| tau | 单步位置 P99 / 最大 (mm) | 单步姿态 P99 / 最大 (°) | 跟随误差 P95 (mm) | 滞后 |
|---|---|---|---|---|
| 0 | 19.4 / 33.7（左）、21.9 / 40.1（右） | 4.4 / 8.5、5.1 / 9.5 | 0 | 0 |
| 0.01 | 9.6 / 14.7、11.8 / 17.6 | 2.3 / 4.3、2.6 / 4.5 | 11.6、13.3 | 约 5 ms |
| 0.02 | 7.5 / 10.4、9.5 / 12.8 | 1.8 / 3.0、2.1 / 3.3 | 22.6、26.3 | 约 15 ms |

上游约 85 Hz，每 2～3 个输出周期才有新输入；旁路时输出呈"重复—阶跃"，平滑后每周期均匀推进。
该段录制为快速运动，跟随误差为运动中的滞后量，静止后收敛到 0。
tau 按时间定义，与输出频率无关：同一 tau 下频率越高单步变化越小，跟随延迟不变。
默认 `0.02` 约 60 ms 走完阶跃的 95%；需要更低延迟可用 `0.01`。
机械臂实际抖动与操作延迟感仍需在设备上确认。
