...
固定输出路径中，每臂在 `vr_follow_frame` 内对计算目标做一阶平滑，再转换到
current pose 坐标系发布：

- 位置：`p_out = p_prev + alpha * (p_target - p_prev)`
- 姿态：归一化后 `slerp(q_prev, q_target, alpha)`，按最短旋转插值
- `alpha = 1 - exp(-dt / tau)`，`dt` 为该臂上次成功发布至今的单调时间，上限 10 ms（约两个输出周期）

`p_prev/q_prev` 仅在目标成功发布后更新，并同时写入最后命令缓存；恢复暂停或重新进入
UPDATE 时以最后命令为基准，与已发布目标连续。上游没有新输入时继续逼近最新目标，
新输入到达后从当前输出继续，不排队历史目标。进入 UPDATE、禁用、校准、控制拓扑变化、
HOLD→OCS2、输入超时恢复、WBC 单臂重建和双臂解耦重建都会清空平滑状态，
首帧直接输出新基准对应的目标。TF 暂时失败时平滑状态保持不变，恢复后最多按 10 ms 推进一步。

在计算坐标系中平滑，底盘运动时目标仍随车体移动，不会在 world 中拖尾。
旧的冻结帧追赶斜坡已删除；输入超时与重建基准机制不变。

参数 `vr_target_smoothing_tau_s`（double，秒，默认 `0.02`）由 `arms_target_manager_node`
启动时读取，只读，不支持运行时修改。`0` 表示旁路（与第一阶段输出一致），
负值或非有限值导致节点启动失败。

这是目标跟随平滑，会增加延迟，不提供速度、加速度或加加速度限制。

离线回放（bag `20260911_113809`，按定时周期零阶保持重采样已录制的 target，
再应用上述滤波；只反映算法效果，未包含 TF、executor 调度和真实设备）：
