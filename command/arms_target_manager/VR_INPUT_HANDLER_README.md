...
4. 用 **左握把 + 左摇杆四方向** 切换身体模式；用 **右握把 + 右摇杆上/下** 切换双臂耦合 / 底盘锁。单臂启用默认用左 Y / 右 B；**左右握把 + 左 Y + 右 B**（事件 16）可把 Y/B 切到 VR 暂停语义。
...
- 点一下 **左握把**（未被组合键消费的下降沿）在 **XY** 与 **Z+Yaw** 之间切换：Y → 升降，X → 偏航。
- 离开跟随后，腰部摇杆平面自动回到 XY；此时左握把恢复为「切换左臂摇杆 XY / Z+Yaw」。
...
| 右握把 + 右摇杆**右** | 28 | — | 预留，当前不处理 |

约束：

- 双臂已耦合时，禁止通过左 Y / 右 B 单独切换任一臂。
- 任意一臂禁用时，禁止开启双臂耦合。
- 左 Y / 右 B（禁用语义）与事件 25–26 复用同一套 pending / 超时 / 回中逻辑。事件 16 的状态对翻不走这条 pending。
...
6. **组合键抑制**: FULL_BODY 下按住握把会锁存对应摇杆，松手后须回中才恢复末端/底盘轴值
